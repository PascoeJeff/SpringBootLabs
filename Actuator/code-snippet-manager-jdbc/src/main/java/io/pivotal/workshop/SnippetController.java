...
public class SnippetController {
    ...
    public List<SnippetInfo> snippets() {
        System.out.println("In snippets method!");
    ...
    @GetMapping("creationDateRange")
    public List<SnippetInfo> snippetByCreationDateRange(@RequestParam("startDate") String startDate,
                                                        @RequestParam("endDate") String endDate) {
        return snippetRepository.findByCreationDateRange(startDate, endDate).stream()
                .map(snippetPresenter::present)
                .collect(toList());
    }
